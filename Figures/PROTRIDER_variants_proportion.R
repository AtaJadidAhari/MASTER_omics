...
pr_dt[method=="merge_pr_var_res", method := "PROTRIDER"]

pr_dt[method=="zScore_gt5", method := "zScores\n(|zScore| > 5)"]


pr_dt[ , pr_expresssion := "Non-outlier"]
pr_dt[zScore > 0 & aberrant == TRUE, pr_expresssion := "Overexpression"]
pr_dt[zScore < 0 & aberrant == TRUE, pr_expresssion := "Underexpression"]

pr_dt[, total_counts := .N, by = .(method, pr_expresssion)]



plot_dt <- pr_dt[, .N, by = .(method, pr_expresssion, Consequence_most_severe)][
  , prop := N / sum(N), by = .(method, pr_expresssion)]

bar_totals <- plot_dt[, .(total_N = sum(N)), by = .(method, pr_expresssion)]
...
plot_dt <- left_join(plot_dt, bar_totals, by=c("method", "pr_expresssion"))
...
p_n <- ggplot(plot_dt[!Consequence_most_severe %in% no_show_consequence], aes(x=prop, y=method, fill = Consequence_most_severe)) +
  geom_bar(stat = "identity", position = "stack") +
   geom_text(data = bar_totals,
              aes(y = method, x = 0.6, label = paste0("n = ", total_N)),
...
png(paste0("/omics/odcf/analysis/hipo/hipo_021/outlier_analysis/results/res_202507/outlier_proportion_rare_variant_protrider.png") ,
...
png(paste0("/omics/odcf/analysis/hipo/hipo_021/outlier_analysis/results/res_202507/outlier_proportion_rare_variant_protrider_noIntron.png") ,
    width = 10, height = 6, units = "in", res = 600)
p_n
dev.off()
