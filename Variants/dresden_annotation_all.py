...
for vep_res in os.listdir(vcf_base_path):
    ...
    final_df["max_spliceai_score"] = final_df[["SpliceAI_pred_DS_AL", "SpliceAI_pred_DS_AG", 
        "SpliceAI_pred_DS_DG", "SpliceAI_pred_DS_DL"
    ]].max(axis=1)
    
    final_df = final_df.rename(columns={final_df.columns[8]: "sample_blood", final_df.columns[9]: "sample_plasma"})
    
    all_dfs.append(final_df[(final_df["IMPACT"].str.contains("HIGH")) | (final_df["max_spliceai_score"] >= 0.2) | (final_df["Consequence"].str.contains("missense"))])
...
merged_vars = res.rename(columns={"#Uploaded_variation": f"{variant_type}_#Uploaded_variation", "Consequence": f"{variant_type}_Consequence", "IMPACT": f"{variant_type}_IMPACT", "am_pathogenicity": f"{variant_type}_am_pathogenicity","LoF": f"{variant_type}_am_LoF", "max_spliceai_score": f"{variant_type}_max_spliceai_score", })



# merged_vars.to_parquet("/omics/odcf/analysis/hipo/hipo_021/outlier_analysis/vcf/dresden_annotations/germline/germline_damaging_aggregated_all.parquet", index=None)

merged_vars.to_parquet("/omics/odcf/analysis/hipo/hipo_021/outlier_analysis/vcf/dresden_annotations/somatic/somatic_damaging_aggregated_all.parquet", index=None)
